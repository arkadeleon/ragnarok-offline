...
- 新建 `Packets/CZ/PACKET_CZ_PC_PURCHASE_ITEMLIST.swift`：只放 `public let HEADER_CZ_PC_PURCHASE_ITEMLIST: Int16 = 0xc8`（struct 已生成）。
- `PacketRegistryTests` 现有测试会覆盖新注册项，跑一下确认。
...
- `CZ_PC_PURCHASE_ITEMLIST(items: [(itemID: Int, amount: Int)])` → `packetLength = 4 + items.count * PACKET_CZ_PC_PURCHASE_ITEMLIST_sub.size`。
- `CZ_PC_SELL_ITEMLIST(items: [(index: Int, amount: Int)])` → 同上，`packetType = HEADER_CZ_PC_SELL_ITEMLIST`。
- `CZ_NPC_TRADE_QUIT()`。

参数类型用第 3 步的模型（`NPCStorePurchase` / `NPCStoreSale`）而不是元组，和 `CZ_SHORTCUT_KEY_CHANGE2(change: ShortcutChange)` 的做法一致。

### 3. RagnarokModels：Core Model

新建 `Packages/RagnarokModels/Sources/RagnarokModels/NPCStore.swift`（仿 `WarpList.swift`）：

```swift
public enum NPCStoreDealType: UInt8, Sendable { case buy = 0, sell = 1 }

public struct NPCStoreBuyItem: Sendable, Hashable {
...
public struct NPCStoreSellItem: Sendable, Hashable {
...
public enum NPCStore: Sendable {
    case buy([NPCStoreBuyItem])   // init(from: PACKET_ZC_PC_PURCHASE_ITEMLIST)
    case sell([NPCStoreSellItem]) // init(from: PACKET_ZC_PC_SELL_ITEMLIST)
}

public struct NPCStorePurchase: Sendable { public let itemID: Int; public let amount: Int }
public struct NPCStoreSale: Sendable { public let index: Int; public let amount: Int }
```
...
- 属性（`warpList` 旁）：`var dealSelectionNPCID: UInt32?`、`var npcStore: NPCStore?`。
- `handleMapPacket` 加分支（放在 NPC 对话那组附近）：
  - `PACKET_ZC_SELECT_DEALTYPE` → `dealSelectionNPCID = packet.npcId`
  - `PACKET_ZC_PC_PURCHASE_ITEMLIST` → `npcStore = .buy(...)`
  - `PACKET_ZC_PC_SELL_ITEMLIST` → `npcStore = .sell(...)`
  - `PACKET_ZC_PC_PURCHASE_RESULT` → `npcStore = nil`；`context.messageCenter.addMessage(for: packet)`
  - `PACKET_ZC_PC_SELL_RESULT` → 同上
- `// MARK: - NPC` 段新增方法：
  - `selectDealType(_ type: NPCStoreDealType)`：guard `dealSelectionNPCID`，发 `CZ_ACK_SELECT_DEALTYPE`，清 `dealSelectionNPCID`。
  - `cancelDealSelection()`：只清 `dealSelectionNPCID`（roBrowser 不发包）。
  - `purchaseItems(_ purchases: [NPCStorePurchase])` / `sellItems(_ sales: [NPCStoreSale])`：发对应包；不清 `npcStore`，等结果包再关（与 roBrowser 一致，结果包决定关窗与提示）。
  - `closeNPCStore()`：`npcStore = nil` 并发 `CZ_NPC_TRADE_QUIT`。
- `exitCurrentPhase()` / 换图时不必特殊处理：`npcStore` 是普通可选属性，随 `.map` 阶段退出时一并清掉即可（在 `case .map:` 里顺手置 nil，`dialog`/`warpList` 目前也没清，可一起补）。
...
**`UI/NPCStoreDealTypeView.swift`（新建，小）**：基于 `MessageBoxView(msg 92)`，bottomBar 三个 `.buttonStyle(.game)` 按钮 Buy / Sell / Cancel（尺寸同 `WarpListView` 的 42×20），分别调用 `gameSession.selectDealType(.buy / .sell)`、`cancelDealSelection()`。

**`UI/NPCStoreView.swift`（新建，主窗口）**，`var store: NPCStore`，`@Environment(GameSession.self)`、`@Environment(GameContext.self)`：
- 布局：`GameWindow`，宽 280（与 NPC 对话/传送列表一致），`GameTitleBar(closeAction: gameSession.closeNPCStore)`。内容自上而下：
  1. 标题行（msg 186 / 185）+ 商品列表 `ScrollView`（高约 130，参考 `NPCDialogMessageBox`）。每行：32×32 图标（`resourceManager.itemIconImage(forItemID:)`，写成 file-private 的 `NPCStoreItemRow`）、名称（`itemInfoTable.localizedIdentifiedItemName(forItemID:)`，缺省显示 ID）、单价「N Zeny」；卖出模式额外显示背包持有数量。
...
- bottomBar：`GameBottomBar` 里「Buy」/「Sell」（购物车为空时 disabled）+「Cancel」→ `closeNPCStore()`。提交时把 `cart` 映射为 `[NPCStorePurchase]` / `[NPCStoreSale]`。
- `.onChange(of: store)` 时清空 `cart`（同 `WarpListView.onChange(of: warpList.mapNames)`）。给 `NPCStore` 加 `Equatable`。
...
```swift
if let npcStore = gameSession.npcStore { NPCStoreView(store: npcStore) }
if let npcID = gameSession.dealSelectionNPCID { NPCStoreDealTypeView() }
```
顺序放在 `NPCDialogView` 之后，保证 `callshop` 时商店盖在对话框上。

## 关键文件

- 改：`Packages/RagnarokPackets/Sources/RagnarokPackets/PacketRegistry.swift`（取消注释 0xcb）
- 新：`Packages/RagnarokPackets/.../Packets/ZC/PACKET_ZC_PC_SELL_RESULT.swift`、`Packets/CZ/PACKET_CZ_NPC_TRADE_QUIT.swift`、`Packets/CZ/PACKET_CZ_PC_PURCHASE_ITEMLIST.swift`
- 改：`Packages/RagnarokNetwork/Sources/RagnarokNetwork/PacketFactory.swift`
- 新：`Packages/RagnarokModels/Sources/RagnarokModels/NPCStore.swift`
...
- 新：`Packages/RagnarokGame/Sources/RagnarokGame/UI/NPCStoreView.swift`、`UI/NPCStoreDealTypeView.swift`

## 验证

1. `swift test --package-path Packages/RagnarokPackets`（`PacketRegistryTests` 覆盖新注册的 0xcb）；`swift build --package-path Packages/RagnarokGame` 确认三个包编译通过（macOS 目标）。
2. Xcode 预览：`NPCStoreView` 买/卖两个 Preview、`NPCStoreDealTypeView` Preview 能渲染。
