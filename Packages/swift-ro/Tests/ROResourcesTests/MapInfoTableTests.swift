...
final class MapInfoTableTests: XCTestCase {
    func testChineseSimplified() async throws {
        ...
        let mapLocalization = MapInfoTable(locale: locale)
    ...
    func testBGM() {
        let mapLocalization = MapInfoTable(locale: .current)
        let prontera = mapLocalization.mapBGM(forMapName: "prontera")
        XCTAssertEqual(prontera, "08.mp3")
    }
}
