...
final class NPCDatabaseTests: XCTestCase {
    func testPrerenewal() async throws {
        ...
        let scriptCommands = try await npcDatabase.scriptCommands()
        ...
        let poringMonsterSpawns = scriptCommands.monsterSpawns(for: poring)
        XCTAssertEqual(poringMonsterSpawns.count, 28)

        let prtfild08MonsterSpawns = scriptCommands.monsterSpawns(forMapName: "prt_fild08")
