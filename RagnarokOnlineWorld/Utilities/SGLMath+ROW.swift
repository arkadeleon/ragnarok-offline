...
extension SGLMath {
    ...
    static func calcNormal<T: FloatingPointArithmeticType>(_ a: Vector3<T>, _ b: Vector3<T>, _ c: Vector3<T>) -> Vector3<T> {
        let v1 = c - b
        let v2 = a - b
        let v3 = cross(v1, v2)
        return normalize(v3)
    ...
    static func calcNormal<T: FloatingPointArithmeticType>(_ a: Vector3<T>, _ b: Vector3<T>, _ c: Vector3<T>, _ d: Vector3<T>) -> Vector3<T> {
        var v1 = c - b
        var v2 = a - b
        var v3 = cross(v1, v2)
        var v = normalize(v3)
    ...
    static func translateZ<T>(_ mat: Matrix4x4<T>, _ z: T) -> Matrix4x4<T> {
        var dest = mat
        dest[3] += dest[2] * z
        return dest
    }
    ...
    static func extractRotation<T: FloatingPointArithmeticType>(_ mat: Matrix4x4<T>) -> Matrix4x4<T> {
